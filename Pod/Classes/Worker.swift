...
class Worker {
    ...
    init(client: LogseneClient, type: String, maxOfflineMessages: Int) throws {
        ...
        timer.scheduleRepeating(deadline: .now(), interval: .seconds(timeTriggerSec), leeway: .seconds(1))
    ...
    fileprivate func invalidateTimer() {
        timer.scheduleRepeating(deadline: .now() + .seconds(timeTriggerSec), interval: .seconds(timeTriggerSec), leeway: .seconds(1))
    }
